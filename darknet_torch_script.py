...
class YOLOLayer(jit.ScriptModule):
    """YOLO detection module."""

    __constants__ = ['anchors', 'num_classes', 'bbox_attrs', 'input_size',
                     'gh', 'gw', 'stride', 'na']

    def __init__(self, anchors, num_classes, input_size, gh, gw):
        """
        ...
        self.stride = self.input_size[0] / self.gh
        self.na = len(self.anchors)

        grid_x = torch.arange(self.gw, dtype=torch.float32).repeat(self.gh, 1).view((1, 1, self.gh, self.gw))
        grid_y = torch.arange(self.gh, dtype=torch.float32).repeat(self.gw, 1).t().view((1, 1, self.gh, self.gw))
        scaled_anchors = torch.tensor([(aw / self.stride, ah / self.stride) for aw, ah in self.anchors])
        anchor_w = scaled_anchors[:, 0:1].view((1, self.na, 1, 1))
        anchor_h = scaled_anchors[:, 1:2].view((1, self.na, 1, 1))

        self.grid_x = nn.Parameter(grid_x)
        self.grid_y = nn.Parameter(grid_y)
        self.anchor_h = nn.Parameter(anchor_h)
        self.anchor_w = nn.Parameter(anchor_w)
    ...
    def forward(self, x):
        ...
        prediction = x.view(nb, self.na, self.bbox_attrs, self.gh, self.gw).permute(0, 1, 3, 4, 2).contiguous()
...
def create_modules(module_defs):
    ...
    for i, module_def in enumerate(module_defs):
        ...
        if module_def['type'] == 'convolutional':
            ...
            yolo_layer = YOLOLayer(anchors, num_classes, (height, width), gh, gw)
...
if __name__ == '__main__':
    d = Darknet('./model_store/yolo_v3_face.cfg', (416, 416))
    d.load_weights('./model_store/yolo_v3_face_16000.weights')
    d.eval()
    # d = d.cuda()
    d.forward(torch.randn(1, 3, 416, 416))

    print('Trace...')
    inputs = torch.randn(1, 3, 416, 416)
    # inputs = inputs.cuda()
    traced = jit.trace(d.forward, (inputs,))

    import timeit
    print('Test...')
    # inputs = inputs.cuda()
    # traced = traced.cuda()
    traced.forward(inputs)
    print(timeit.timeit(lambda: traced.forward(inputs), number=10))
    d = Darknet('./model_store/yolo_v3_face.cfg', (416, 416))
    d.load_weights('./model_store/yolo_v3_face_16000.weights')
    d.eval()
    # d = d.cuda()
    d.forward(inputs)
    print(timeit.timeit(lambda: d.forward(inputs), number=10))
